...
(
  SELECT
      ...
      AND     GOREMAL_EMAL_CODE = 'ESF') as SIRDPCL_INSTR_EMAIL
...
/*
           AND SGRADVR.SGRADVR_TERM_CODE_EFF = (SELECT MAX(SGRADVR_TERM_CODE_EFF)
                                               ...
                                               AND     ADVR.SGRADVR_PRIM_IND = 'Y'
                                               AND     ADVR.SGRADVR_TERM_CODE_EFF <= '&TERM')
           AND SGRADVR.SGRADVR_SURROGATE_ID = (SELECT MAX(SGRADVR_SURROGATE_ID)
                                              FROM    SGRADVR ADVR
                                              WHERE   ADVR.SGRADVR_PIDM = SGRADVR.SGRADVR_PIDM
                                              AND     ADVR.SGRADVR_TERM_CODE_EFF = SGRADVR.SGRADVR_TERM_CODE_EFF
...
--    AND 'Y' = F_REGISTERED_THIS_TERM(SPRIDEN_PIDM, '&TERM')
--    AND EXISTS (SELECT * FROM SFAREGS WHERE SFRSTCR_PIDM = SPRIDEN_PIDM AND SFRSTCR_TERM_CODE = '&TERM' AND SFRSTCR_RSTS_CODE IN ('RE', 'RW'))
--    AND EXISTS (SELECT * FROM SFAREGS WHERE SFRSTCR_PIDM = SPRIDEN_PIDM AND SFRSTCR_TERM_CODE = '&TERM' AND SFRSTCR_RSTS_CODE IN (SELECT STVRSTS_CODE FROM STVRSTS))
